  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start gap-4">
        <div className="relative w-32 h-32 rounded-xl bg-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0">
          {character.avatar ? (
            // eslint-disable-next-line @next/next/no-img-element
        ...
        <div className="mt-1 flex gap-2">
          {multiline ? (
            <textarea
              ...
              onBlur={commit}
              className="flex-1 px-2 py-1.5 rounded-lg border border-[var(--color-border)] outline-none text-sm focus:border-[var(--color-primary)] resize-none"
            />
          ) : (
            <input
              autoFocus
              ...
              className="flex-1 px-2 py-1.5 rounded-lg border border-[var(--color-border)] outline-none text-sm focus:border-[var(--color-primary)]"
            />
          )}
        </div>
      ) : (
        <div className="mt-1 text-sm leading-relaxed whitespace-pre-wrap min-h-[1.25rem]">
          {value || <span className="text-gray-400">{placeholder ?? '点击编辑'}</span>}
        </div>
      )}
